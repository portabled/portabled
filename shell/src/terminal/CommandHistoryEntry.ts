namespace terminal {
  ...
  export class CommandHistoryEntry {
    ...
    private _tryStoreSelectionIE(input: HTMLInputElement | HTMLTextAreaElement): boolean {
      ...
      var range: any /* TextRange */ = selection && typeof selection.createRange === 'function' ?
        selection.createRange() : null;

      if (!range || typeof range.parentElement !== 'function' || range.parentElement() !== input) return false;
      ...
      if ((<any>input).createTextRange) {
        ...
        var textInputRange = (<any>input).createTextRange();
        textInputRange.moveToBookmark(range.getBookmark());

        // Check if the start and end of the selection are at the very end
        // of the input, since moveStart/moveEnd doesn't return what we want
        // in those cases
        var endRange = (<any>input).createTextRange();
    ...
    private _tryApplySelectionIE(input: HTMLInputElement | HTMLTextAreaElement): boolean {
      if (!(<any>input).createTextRange) return false;
