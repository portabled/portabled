function overrideWindow(window, drive, cachePath, localStorageCachePath) {

  function WindowOverride() {
  }

  function overrideProp(k) {
    try {
      var origFn = window[k];
  ...
  function overrideDocument(document) {

    function DocumentOverride() {
    }

    function proxyProperty(k) {
      try {
        var val = document[k];
  ...
  function overrideXHR(drive, cachePath) {
    ...
    XMLHttpRequestOverride.prototype = {
      ...
      send: function() {
        ...
        setTimeout(function() {
          ...
          if (/\.js$/.test(path)) this.responseText += '//'+'# '+'sourceURL='+cleanUrl;
