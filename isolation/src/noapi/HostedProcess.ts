class HostedProcess {
  ...
  enhanceChildProcess: (proc: HostedProcess) => void = null;
  ondispose: () => void = null;
  ...
  constructor(
    ...
    var enhChild = (proc) => {
      if (this.enhanceChildProcess)
        this.enhanceChildProcess(proc);
      proc.enhanceChildProcess = enhChild;
    };
    this.coreModules.child_process = createChildProcess(
      this._drive,
      window,
      {
