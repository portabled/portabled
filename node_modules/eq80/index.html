...
function copy() {
    ...
    function run() {
        ...
        if (isOutputDir) {
            ...
            for (var i = 0; i < inputAllFiles.length; i++) {
                ...
                if (targetPath.slice(-1) === '/' || targetPath.slice(-1) === '\\')
                    continue;
            ...
            if (fs.existsSync(outputFile)) {
                ...
                var resultHTML = updateParsedHTMLContent(outputFileHTML, parsedOutputFile, inputAllFiles, function (file) { return fs.readFileSync(file.contentPath) + ''; });
        ...
        function copyAll() {
            ...
            if (parsedShells.length) {
                ...
                for (var i = 0; i < parsedShells.length; i++) {
                    if (parsedShells[i].path === latestParsed.path)
                        continue;
                    ...
                    var updatedHTML = updateParsedHTMLContent(latestParsed.content, latestParsed, allFiles, function (entry) {
            ...
            function extractCompressedDocWrite(content, callback) {
                ...
                while (pos < content.length) {
                    ...
                    try {
                        ...
                        if (callCount && typeof result === 'string')
                            callback(result);
                    }
                    catch (err) { }
                }
    ...
    function updateParsedHTMLContent(outputCompleteHTML, outputParsed, inputAllFiles, readInputContent, deleteExistingFiles) {
        ...
        for (var i = 0; i < reverseFiles.length; i++) {
            ...
            if (matchingInputFile) {
                ...
                if (!content && typeof content !== 'string')
                    content = readInputContent(matchingInputFile) + '';
                resultBuf.unshift('<!--' +
        ...
        for (var i = 0; i < inputAllFiles.length; i++) {
            ...
            if (matchingOutputFile)
                continue; // that's already taken care of
            var content = newFile.content;
            if (!content && typeof content !== 'string')
                content = readInputContent(newFile); // fs.readFileSync(newFile.contentPath)+'';
...
function loader(window, document) {
	...
	loader.build = {
    timestamp: 1529567061101, // Thu Jun 21 2018 08:44:21 GMT+0100 (BST)
    taken: 7491,
    platform: "node v8.10.0 on darwin/x64"
}
...
function persistence(document, uniqueKey, optionalDrives) {
	...
	persistence.build = {
    timestamp: 1528491644901, // Fri Jun 08 2018 22:00:44 GMT+0100 (BST)
    taken: 4387,
    platform: "node v8.10.0 on darwin/x64"
}
...
;
  ...
  var fs = require("fs"), path = require("path");function buildStats() {
    var buildStartTime = +new Date();
    return completeBuildStats;
    function completeBuildStats() {
        var platform;
        try {
            platform = require('nowindow').navigator.userAgent;
        }
        catch (error) {
...
function compileTS() {
    ...
    function readDirectory(dir, extensions, basePaths, excludePattern, includeFilePattern, includeDirectoryPattern) {
        var result = [];
        var incReg = includeFilePattern ? new RegExp(includeFilePattern) : { test: function (file) { return true; } };
        var excReg = excludePattern ? new RegExp(excludePattern) : { test: function (file) { return false; } };
        var seen = {};
...
function functionBody(fn, replacements) {
    ...
    if (!replacements)
        ...
        return substitute(bodyText, replacements);
}
function getFiles(filelist) {
    var logicalPathList = [];
    var map = {};
    var logicalPathToFilesystemPath = {};
    ...
    function addFiles(files, skipPathStrLength) {
        if (!files)
            return;
        ...
        if (typeof files === 'object') {
            for (var k in files)
                if (k.charCodeAt(0) === 47 && files.hasOwnProperty(k)) { // k is path thus starts with slash
                    var content = files[k];
                    if (typeof content === 'string' || typeof content === 'function') {
                        addFileContent(k, content);
                    }
                    else if (content && typeof content.length === 'number') { // array inside property
                        for (var i = 0; i < content.length; i++) {
    ...
    function addFileContent(logicalPath, content) {
        delete logicalPathToFilesystemPath[logicalPath];
        if (!map[logicalPath])
            logicalPathList.push(logicalPath);
    ...
    function addFileContentRead(file, skipPathStrLength, redirectRoot) {
        ...
        if (newPath.charCodeAt(0) !== 47)
            newPath = '/' + newPath;
        if (filelist.filterFiles) {
            if (!filelist.filterFiles(newPath, file))
                return;
        }
...
function htmlSpotExternals(html) {
    ...
    var linkStart = /(\<script[\s\>])|(\<link[\s\>])|(\<\!\-\-)|(\<style[\s\>])/gim;
        ...
        if (match[1]) {
            ...
            if (src) {
                result.push({
                    ...
                    substituteLead: openingTagText.slice(0, srcMatch.index +
            ...
            if (href && /\srel=((\"stylesheet\")|(\'stylesheet\')|(stylesheet\b))/i.test(linkTagText)) {
                var styleLead = '<style' +
                    linkTagText.slice(5 /* "<link".length */, hrefMatch.index +
...
function jsStringLong(str) {
    ...
    return [
        ...
        'm*=' + (25 + 26) + ';',
        '}',
        'if (nn<100 && !r[nn]) return r[nn] = d[nn];',
        'else return d[nn];',
        '});\n',
...
function substitute(bodyText, replacements) {
    if (typeof replacements === 'function') {
        var update = bodyText.replace(/\"\#([\S]+)\#\"/g, function (str, token) {
...
function wrapEQ80(options) {
    ...
    return result;
    function checkBlownLines(html) {
        var i = null;
        var lines = html.split('\n');
        var strangeLines = [];
        var strangeStretches = [];
            ...
            for (var iStretch = 0; iStretch < strangeStretches.length; iStretch++) {
                ...
                for (var iLineAfter = 0; iLineAfter < 10; iLineAfter++) {
                    ...
                    if (lnum > lines.length)
                        break;
                    if (iLineAfter + 1 < strangeStretches.length && lnum >= strangeStretches[iLineAfter + 1].start - 3)
                        break;
                    console.log('[' + lnum + '] ' + lines[lnum]);
                }
            }
...
function wrapScript(replacements) {
    var result = functionBody((template_content + '').replace(/\<\/\[script/g, '</' + 'script').replace(/\"\#build_functions\#\"/, functionBody + '\n' +
        functionArgs + '\n' +
        jsString + '\n' +
        jsStringLong + '\n'), replacements);
...
function template_content() {
    ...
    function runTests(tests) {
        ...
        if (testFilter && testFilter.charAt(0) === '#')
            testFilter = testFilter.slice(1);
        var summary = document.createElement('h2');
        summary.style.color = 'black';
        if ('textContent' in summary)
            summary.textContent = 'Tests (total ' + tests.length + '):';
        else
            summary.innerText = tests.length + 'Tests (total ' + tests.length + '):';
        var successCount = 0;
        var failCount = 0;
        ...
        function continueRunTests() {
            ...
            while (true) {
                ...
                t.run(function (error) {
                    ...
                    if ('textContent' in tm)
                        tm.textContent = ' ' + (finish - start) + 'ms';
                    else
                        tm.innerText = ' ' + (finish - start) + 'ms';
        ...
        function addTestRow(i) {
            ...
            if ('textContent' in testEntry)
                testEntry.textContent = t.name;
            else
                testEntry.innerText = t.name;
            t.testEntry = testEntry;
        }
    }
    function assert(condition, message) {
        if (!condition)
            throw new Error(message || 'Failure ' + condition);
    }
    assert.equal = (function () {
        function equal(expected, actual, message) {
            if (expected != actual)
                throw new Error(message || 'Unmatch: ' + expected + ' != ' + actual);
        }
        return equal;
    })();
    function generateTests() {
        ...
        var _dummy = {};
        for (var k in tests)
            if (!(k in _dummy) && tests[k] && /^[a-z\_]/.test(k)) {
...
(function (tests) {
    tests.buildMetadata = {
        'not null': function () { return assert(loader.build); },
        'timestamp>=1462227529476': function () { return assert(loader.build.timestamp >= 1462227529476); },
        '60000>taken>10': function () { return assert(loader.build.taken > 10 && loader.build.taken < 60000); },
        'platform is string': function () { return assert.equal('string', typeof loader.build.platform); }
    };
})(tests || (tests = {}));
;
var tests;
(function (tests) {
    tests.deriveUniqueKey_tests = {
        null_value: function () { return assertGoodKey(loader.deriveUniqueKey(null)); },
        strempty_value: function () { return assertGoodKey(loader.deriveUniqueKey('')); },
        str123456_value: function () { return assertGoodKey(loader.deriveUniqueKey('123456')); }
    };
    function assertGoodKey(key) {
        ...
        for (var i = 3; i < key.length; i++) {
            assert(key.charAt(i) !== key.charAt(i - 1)
                || key.charAt(i - 1) !== key.charAt(i - 2)
                || key.charAt(i - 2) !== key.charAt(i - 3), '3 or more duplicate characters at ' + (i - 3));
        }
    }
})(tests || (tests = {}));
// (triple slash <) reference path="../src/API.d.ts"/>
;
        // running tests from HTML page
        var ui = document.createElement('div');
        ui.style.color = 'black';
